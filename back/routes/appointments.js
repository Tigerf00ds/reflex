...
router.post('/take', async (req, res) => {
    ...
    const sql2 = 'SELECT * FROM banned_users WHERE telephone = ?';
    db.query(sql2, [he.encode(telephone)], (err, results) => {
        ...
        if(results.length>0) {
            console.error('Numéro de téléphone banni.');
            return res.status(400).json({ error: 'Erreur requête', details: 'Numéro de téléphone banni.' });
        }
        const sql3 = 'SELECT * FROM appointments WHERE date_booked = ?';
        db.query(sql3, [he.encode(date_booked)], (err, results) => {
            if(err){
                return res.status(500).json({ error: 'Erreur serveur', details: err });
            }
            const overlapping = results.some(result => 
            ...
            const sql4 = 'INSERT INTO appointments (care_id, address, name, email, telephone, date_booked, time_depart, time_start, time_end, time_return, price) VALUES (?,?,?,?,?,?,?,?,?,?,?)';
            db.query(sql4, [care_id, he.encode(address), he.encode(name), he.encode(email), he.encode(telephone), he.encode(date_booked), time_depart, time_start, time_end, time_return, price], (err, results) => {
                if (err) {
                    console.error('Erreur SQL :', err);
                    return res.status(500).json({ error: 'Erreur serveur', details: err });
                }
